...
class CopywritingApproach(Approach):
    ...
    def __init__(self, dalle2helper: Dalle2Helper, api_key: str, api_base: str, api_version: str, chatgpt_deployment_name: str) -> None:
    ...
    def run(self, title: str, titleDescription: str, overrides: dict) -> any:
        ...
        if (useDalle):
            copywrite = self._insert_image(copywrite,artStyle)
        return {
            "copywriteHTML": copywrite,
        }

    def _get_image(self, text, style):
    ...
    def _insert_image(self, html_text, art_style):
        # 查找所有包含[IMG]的<p>标签
        ...
        for p_tag in p_tags_with_img:
            img_replacement = self._get_image(p_tag, " ".join(art_style))
